...
to Setup
  ca
  set Values n-values NObjetos [random 50]
  set Weights n-values NObjetos [random 100]
  set Maxobjs n-values NObjetos [(random 10) + 1]
  set obligatorio n-values NObjetos [random 2]
  let minwei sum (map * obligatorio Weights) + 1
  set T-max (random 250) + minwei
  output-print "Problem:"
  output-print "--------"
  output-print (word "Values:  " Values)
  output-print (word "Weights: " Weights)
  output-print (word "Maxobjs: " Maxobjs)
  output-print (word "obligat: " obligatorio)
...
to AI:Initial-Population [#population]
  create-AI:individuals #population [
    set content n-values NObjetos [(random 10)]
    AI:Compute-fitness
    hide-turtle
  ]
end
...
to AI:Compute-fitness
  ...
  let objetosdemas 0
  let oblg 0
  let difpeso wei - T-max
  while[i < length content][
    let numobj item i content
    let maxob item i Maxobjs
    let obligat item i obligatorio
    if numobj > maxob[set objetosdemas objetosdemas + (numobj - maxob)]
    if numobj < obligat [set oblg oblg + 1]
    set i i + 1
  ]

  if difpeso < 0 [set difpeso 0]
  ifelse (wei > T-max) or (objetosdemas > 0) or (oblg > 0)[
  set fitness  val - ((10 * difpeso)+(10 * objetosdemas * objetosdemas)+(10 * oblg * oblg))]
  [set fitness val]
